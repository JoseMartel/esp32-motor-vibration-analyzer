...
#include <string.h>
#include <sys/param.h>
#ifdef OTA_WITH_CRYPTO
#include "esp_crypto.h"
#endif

#define IMAGE_HEADER_SIZE                                                     \
	sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t)           \
		+ sizeof(esp_app_desc_t) + 1
#define DEFAULT_OTA_BUF_SIZE (IMAGE_HEADER_SIZE - 1)
...
static esp_err_t
_http_handle_response_code(esp_http_client_handle_t http_client,
						   int status_code) {
	esp_err_t err;
	if (status_code == HttpStatus_MovedPermanently
		|| status_code == HttpStatus_Found
		...
		esp_http_client_add_auth(http_client);
			   ...
			   || status_code == HttpStatus_Forbidden) {
	...
	if (process_again(status_code)) {
		while (1) {
			 ...
			 */
			int data_read = esp_http_client_read(http_client,
												 upgrade_data_buf,
...
static esp_err_t
_http_connect(esp_http_client_handle_t http_client) {
	...
	do {
		...
		if (post_len) {
			...
			while (post_len > 0) {
				write_len
					= esp_http_client_write(http_client, post_data, post_len);
				if (write_len < 0) {
...
static esp_err_t
_ota_write(esp_https_ota_t *https_ota_handle, const void *buffer,
		   size_t buf_len) {
	if (buffer == NULL || https_ota_handle == NULL) {
		return ESP_FAIL;
	}
	esp_err_t err
		= esp_ota_write(https_ota_handle->update_handle, buffer, buf_len);
	if (err != ESP_OK) {
		ESP_LOGE(TAG, "Error: esp_ota_write failed! err=0x%x", err);
	} else {
		https_ota_handle->binary_file_len += buf_len;
		ESP_LOGD(TAG,
				 "Written image length %d",
				 https_ota_handle->binary_file_len);
...
static bool
is_server_verification_enabled(esp_https_ota_config_t *ota_config) {
...
esp_err_t
esp_https_ota_begin(esp_https_ota_config_t *ota_config,
					esp_https_ota_handle_t *handle) {
	esp_err_t err;

	if (handle == NULL || ota_config == NULL
	...
	if (!is_server_verification_enabled(ota_config)) {
		...
		ESP_LOGW(TAG,
				 "Continuing with insecure option because "
				 "CONFIG_OTA_ALLOW_HTTP is set.");
#else
		ESP_LOGE(TAG,
				 "No option for server verification is enabled in "
				 "esp_http_client config.");
		*handle = NULL;
		return ESP_ERR_INVALID_ARG;
#endif
	}
	...
	https_ota_handle->partial_http_download
		= ota_config->partial_http_download;
	https_ota_handle->max_http_request_size
		= (ota_config->max_http_request_size == 0)
			  ? DEFAULT_REQUEST_SIZE
			  : ota_config->max_http_request_size;

	/* Initiate HTTP Connection */
	https_ota_handle->http_client
		= esp_http_client_init(ota_config->http_config);
	...
	if (https_ota_handle->partial_http_download) {
		esp_http_client_set_method(https_ota_handle->http_client,
								   HTTP_METHOD_HEAD);
		err = esp_http_client_perform(https_ota_handle->http_client);
		if (err != ESP_OK
			|| esp_http_client_get_status_code(https_ota_handle->http_client)
				   != HttpStatus_Ok) {
			ESP_LOGE(TAG, "Failed to get image length");
			goto http_cleanup;
		}
		https_ota_handle->image_length = esp_http_client_get_content_length(
			https_ota_handle->http_client);
		esp_http_client_close(https_ota_handle->http_client);

		if (https_ota_handle->image_length
			> https_ota_handle->max_http_request_size) {
			char *header_val = NULL;
			asprintf(&header_val,
					 "bytes=0-%d",
					 https_ota_handle->max_http_request_size - 1);
			if (header_val == NULL) {
	...
	ESP_LOGI(TAG,
			 "Writing to partition subtype %d at offset 0x%d",
			 https_ota_handle->update_partition->subtype,
			 (int)https_ota_handle->update_partition->address);

	const int alloc_size
		= MAX(ota_config->http_config->buffer_size, DEFAULT_OTA_BUF_SIZE);
...
// esp_err_t
// esp_https_ota_get_img_desc(esp_https_ota_handle_t https_ota_handle,
...
// 	/*
// 	 * while loop is added to download complete image headers, even if the
// 	 * headers are not sent in a single packet.
// 	 */
// 	while (
// 		data_read_size > 0
// 		&& !esp_http_client_is_complete_data_received(handle->http_client)) {
// 		data_read
// 			= esp_http_client_read(handle->http_client,
// 								   (handle->ota_upgrade_buf + bytes_read),
// 								   data_read_size);
// 		/*
// 		 * As esp_http_client_read doesn't return negative error code if select
// 		 * fails, we rely on `errno` to check for underlying transport
// 		 * connectivity closure if any
// 		 */
...
// 		}
// 		//BLAAAAA
// 		char __temp[DEFAULT_OTA_BUF_SIZE];
// 		// printf("def: %d | data:
// 		// %d\n",DEFAULT_OTA_BUF_SIZE,data_read);
// 		api_aes_decrypt((const void *)handle->ota_upgrade_buf,
// 						data_read,
// 						__temp);
...
// 	memcpy(new_app_info,
// 		   &handle->ota_upgrade_buf[sizeof(esp_image_header_t)
...
esp_err_t
esp_https_ota_perform(esp_https_ota_handle_t https_ota_handle) {
	...
	switch (handle->state) {
		case ESP_HTTPS_OTA_BEGIN:
			err = esp_ota_begin(handle->update_partition,
								erase_size,
			...
			/* In case `esp_https_ota_read_img_desc` was invoked first,
			   then the image data read there should be written to OTA
			   partition
			   */
			if (handle->binary_file_len) {
				/*
				 * Header length gets added to handle->binary_file_len in
				 * _ota_write Clear handle->binary_file_len to avoid additional
				 * 289 bytes in binary_file_len
				 */
				int binary_file_len = handle->binary_file_len;
				handle->binary_file_len = 0;
				return _ota_write(handle,
								  (const void *)handle->ota_upgrade_buf,
								  binary_file_len);
			}
			/* falls through */
		case ESP_HTTPS_OTA_IN_PROGRESS:
			...
			// #ifdef OTA_WITH_CRYPTO
			// 			extern esp_err_t api_aes_decrypt(char *input,
			// 											 unsigned input_len,
			// 											 char *output);
			// #endif
			if (data_read == 0) {
				/*
				 *  esp_http_client_is_complete_data_received is added to check
				 * whether complete image is received.
				 */
				bool is_recv_complete
					= esp_http_client_is_complete_data_received(
						handle->http_client);
				/*
				 * As esp_http_client_read doesn't return negative error code
				 * if select fails, we rely on `errno` to check for underlying
				 * transport connectivity closure if any. Incase the complete
				 * data has not been received but the server has sent an
				 * ENOTCONN or ECONNRESET, failure is returned. We close with
				 * success if complete data has been received.
				 */
				...
				api_aes_decrypt((const void *)handle->ota_upgrade_buf,
								data_read,
								__temp);
				return _ota_write(handle, (const void *)__temp, data_read);
#else
				return _ota_write(handle,
								  (const void *)handle->ota_upgrade_buf,
								  data_read);
	...
	if (handle->partial_http_download) {
		if (handle->state == ESP_HTTPS_OTA_IN_PROGRESS
			...
			if ((handle->image_length - handle->binary_file_len)
				> handle->max_http_request_size) {
				asprintf(&header_val,
						 "bytes=%d-%d",
						 handle->binary_file_len,
						 (handle->binary_file_len
						  + handle->max_http_request_size - 1));
			} else {
			...
			esp_http_client_set_header(handle->http_client,
									   "Range",
									   header_val);
			free(header_val);
...
esp_err_t
esp_https_ota_finish(esp_https_ota_handle_t https_ota_handle) {
	esp_https_ota_t *handle = (esp_https_ota_t *)https_ota_handle;
	if (handle == NULL) {
		return ESP_ERR_INVALID_ARG;
	}
	...
	esp_ota_get_partition_description(handle->update_partition, &app_desc);
	ESP_LOGI(TAG,"NEW VERSION: %s",app_desc.version);
	bool check = check_version((uint8_t *)app_desc.version);
	printf("Version is higher? A: %d\n",check);
	if ((err == ESP_OK) && (handle->state == ESP_HTTPS_OTA_SUCCESS) &&(check)) {
	...
	{
		ESP_LOGW(TAG, "Device not updated");
		return ESP_FAIL;
	}
...
esp_err_t
esp_https_ota_abort(esp_https_ota_handle_t https_ota_handle) {
	esp_https_ota_t *handle = (esp_https_ota_t *)https_ota_handle;
	if (handle == NULL) {
		return ESP_ERR_INVALID_ARG;
	}
...
int
esp_https_ota_get_image_len_read(esp_https_ota_handle_t https_ota_handle) {
	esp_https_ota_t *handle = (esp_https_ota_t *)https_ota_handle;
	if (handle == NULL) {
		return -1;
	}
...
esp_err_t
esp_https_ota(const esp_http_client_config_t *config) {
	if (!config) {
